 ...
 * @param end 
 */
export const setCardStatesInRange = (deck: HTMLElement[], start: number, end: number, noCssTransition = 'disable-transition') => {
...
const setSelectedNavIcons =
  (card: HTMLElement | null, selected: boolean, iconSelector = '.nav-icon', cssSelected = 'nav-icon-selected') => {
    if (card === null) return;
